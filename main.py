...
def create_user():
    if request.method == 'POST':
        user = User(name=request.form['user-name'], password=pbkdf2_sha256.hash(request.form['user-password']))
        user.save()
        return redirect(url_for('all_tasks'))
    else:
        return render_template('make_user.jinja2')
...
def incomplete_tasks():
    ...
    if request.method == 'POST':
        user = User.select().where(User.name == request.form['name']).get()
        task_id = Task.update(performed=datetime.now(), performed_by=user)\
